...
**What one PE computes.** Each PE computes `result = weight_i × sum(pixels[0..8])` — i.e., a single weight applied to all 9 spatial positions of one 3×3 input patch. This is the weight-stationary inner loop of a depth-wise or point-wise convolution style accumulation. Four PEs in parallel compute four such weighted sums simultaneously, one per output channel. See Figure 1 (block diagram) and Figure 2 (dataflow diagram).
